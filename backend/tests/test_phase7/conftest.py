"""Phase 7 test fixtures — extends Phase 6 setup for chat route tests.

Reuses the same session-scoped engine, StubProvider, and dependency-override
pattern from Phase 6, so chat routes work against the same test DB.
"""
...
def overridden_client(
    ...
    """TestClient with all Phase 7 dependencies overridden."""
    maker = sessionmaker(bind=_engine)
